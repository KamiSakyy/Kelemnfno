...
public final class DownloadsScreen extends LinearLayout {
    ...
    public DownloadsScreen(Activity a){super(a);activity=a;hub=YoruApp.app().downloads();setOrientation(VERTICAL);setPadding(Ui.dp(a,17),Ui.dp(a,15),Ui.dp(a,17),Ui.dp(a,8));addView(Ui.label(a,"ОФЛАЙН-БИБЛИОТЕКА"));Ui.space(this,10);addView(Ui.text(a,"Загрузки",28,Ui.TEXT,true));Ui.space(this,9);addView(Ui.text(a,"Ð¡ÐºÐ°ÑÐ°Ð½Ð½ÑÐµ ÑÐµÑÐ¸Ð¸ Ñ Ð¿ÑÐµÐ²ÑÑ, ÐºÐ°ÑÐµÑÑÐ²Ð¾Ð¼ Ð¸ Ð±ÑÑÑÑÑÐ¼ Ð·Ð°Ð¿ÑÑÐºÐ¾Ð¼ Ð²Ð¾ Ð²ÑÑÑÐ¾ÐµÐ½Ð½Ð¾Ð¼ Ð¿Ð»ÐµÐµÑÐµ Tsuyu",12,Ui.MUTED,false));Ui.space(this,16);summary=Ui.text(a,"",12,Ui.PURPLE,true);addView(summary);Ui.space(this,12);list=new ListView(a);list.setDivider(null);list.setClipToPadding(false);list.setVerticalScrollBarEnabled(false);list.setPadding(0,0,0,Ui.dp(a,15));list.setBackgroundColor(Ui.BG);adapter=new Adapter();list.setAdapter(adapter);empty=Ui.text(a,"ÐÐ´ÐµÑÑ Ð¿Ð¾ÑÐ²ÑÑÑÑ ÑÐºÐ°ÑÐ°Ð½Ð½ÑÐµ ÑÐµÑÐ¸Ð¸\n\nÐÑÐºÑÐ¾Ð¹ÑÐµ Ð°Ð½Ð¸Ð¼Ðµ Ð¸ Ð½Ð°Ð¶Ð¼Ð¸ÑÐµ Â«Ð¡ÐºÐ°ÑÐ°ÑÑÂ» ÑÑÐ´Ð¾Ð¼ Ñ Ð½ÑÐ¶Ð½Ð¾Ð¹ ÑÐµÑÐ¸ÐµÐ¹ ÐÐ¾ÑÐ¾Ð²ÑÐµ Ð·Ð°Ð³ÑÑÐ·ÐºÐ¸ Ð¾ÑÐºÑÑÐ²Ð°ÑÑÑÑ Ð¿ÑÑÐ¼Ð¾ Ð² Tsuyu",14,Ui.MUTED,false);empty.setPadding(Ui.dp(a,20),Ui.dp(a,40),Ui.dp(a,20),Ui.dp(a,20));empty.setGravity(Gravity.CENTER);FrameLayout frame=new FrameLayout(a);frame.addView(list,new FrameLayout.LayoutParams(-1,-1));frame.addView(empty,new FrameLayout.LayoutParams(-1,-1));addView(frame,new LinearLayout.LayoutParams(-1,0,1));list.setOnItemClickListener((parent,v,pos,id)->{try{if(pos>=0&&pos<rows.size()){Download d=rows.get(pos).download;if(d!=null&&d.state==Download.STATE_COMPLETED){JSONObject m=DownloadHub.metadata(d);Ui.openOffline(activity,d.request.id,Anime.from(m.optJSONObject("anime")),m.optDouble("episode",1));}}}catch(Throwable ignored){}});}
    ...
    private void actions(Download d){if(d==null||d.request==null){Ui.toast(activity,"Эта запись загрузки повреждена");return;}JSONObject meta=DownloadHub.metadata(d);Anime a=Anime.from(meta.optJSONObject("anime"));String voice=meta.optString("voice","");String title=(a.title==null||a.title.isEmpty()?"Загрузка":a.title)+" · серия "+Ui.number(meta.optDouble("episode",1))+(voice.isEmpty()?"":" · "+voice);ArrayList<String> choices=new ArrayList<>();if(d.state==Download.STATE_COMPLETED){choices.add("Ð¡Ð¼Ð¾ÑÑÐµÑÑ Ð² Tsuyu");if(OfflineExporter.canExport(d)){choices.add("Сохранить через проводник");choices.add("Ð¡Ð¾ÑÑÐ°Ð½Ð¸ÑÑ Ð² ÐÐ¸Ð´ÐµÐ¾ / Tsuyu");choices.add("Поделиться файлом");}choices.add("Информация");}else if(d.state==Download.STATE_STOPPED)choices.add("Продолжить");else if(d.state==Download.STATE_FAILED)choices.add("Обновить и повторить");else if(d.state!=Download.STATE_REMOVING)choices.add("Пауза");choices.add("Удалить загрузку");Ui.choices(activity,title,choices.toArray(new String[0]),which->handle(d,a,meta,choices.get(which)));}
    private void handle(Download d,Anime a,JSONObject meta,String choice){try{if(choice.equals("Ð¡Ð¼Ð¾ÑÑÐµÑÑ Ð² Tsuyu"))Ui.openOffline(activity,d.request.id,a,meta.optDouble("episode",1));else if(choice.equals("Сохранить через проводник"))OfflineExporter.saveWithPicker(activity,d);else if(choice.equals("Ð¡Ð¾ÑÑÐ°Ð½Ð¸ÑÑ Ð² ÐÐ¸Ð´ÐµÐ¾ / Tsuyu"))OfflineExporter.saveToMovies(activity,d);else if(choice.equals("Поделиться файлом"))OfflineExporter.share(activity,d);else if(choice.equals("Информация"))Ui.message(activity,"Информация",OfflineExporter.details(d));else if(choice.equals("Продолжить"))hub.resume(d.request.id);else if(choice.equals("Пауза"))hub.pause(d.request.id);else if(choice.equals("Обновить и повторить"))hub.retry(d.request.id);else confirmDelete(d);}catch(Throwable e){Ui.toast(activity,"Действие загрузки не выполнено");}}
    ...
    private final class DownloadRow {
        ...
        void bind(Download d,String resolved) {
            ...
            caption.setText((d.state==Download.STATE_COMPLETED?"▶ ":"")+"Серия "+Ui.number(episode)+(d.state==Download.STATE_COMPLETED?" · готово офлайн":" Â· Ð·Ð°Ð³ÑÑÐ·ÐºÐ° Ð² Tsuyu"));
